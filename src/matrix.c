...
char ** argv;
{
  double	det[1000000],det1=0.,rmse=0.;
  int	n;	/* matrix size */
  int	i,j, doDet=0, doFull=0, readData=0;
  
  for(i=1;i<argc;i++){
    if(!strcmp(argv[i],"-det"))doDet=1;
    else if(!strcmp(argv[i],"-full"))doFull=1;
    else if(!strcmp(argv[i],"-read"))readData=1;
  }
/*
**	read matrix size
*/
  ...
  for(i=0;i<n;i++)for(j=0;j<n;j++)
    fscanf(stdin,"%lf",(input + i*n + j));

  /* if doFull, also read vector */
  if(doFull){
    for(i=0;i<n;i++){
      fscanf(stdin,"%lf",(vectorv + i));
    }
  }

  /* invert matrix */
  Matrix_determinant( input, &det1, n );
  if(fabs(det1)>1e-20){
    Matrix_inversion( input, output, det, n ,parameters);
    if(!doFull){
      /* just print inverse matrix */
      /* print matrix */
      fprintf(stdout,"%d",n);
      if(doDet)fprintf(stdout,"%lf",det1);
      fprintf(stdout,"\n");
      for(i=0;i<n;i++){
	for(j=0;j<n;j++)
	  fprintf(stdout,"%40.20lf ",*(output + i*n + j));
	fprintf(stdout,"\n");
	fflush(stdout);
      }
    }else{
      /* multiply output by vector */
      /*Vector_Matrix_multiplication_to_Vector(vectorv,output,parameters,n);*/
      /* calculate most of rmse 
      ** rmse = P' M P + O.O - 2 V . P
      **
      ** t1 = M P
      ** rmse = (O.O) + P.t1 - 2 V.P
      **
      */
      Vector_Matrix_multiplication_to_Vector(vectorv,output,parameters,n);
      Vector_Matrix_multiplication_to_Vector(parameters,input,t1,n);
      rmse = vDot(t1,parameters,n) - 2. * vDot(vectorv,parameters,n);
      fprintf(stdout,"%d ",n);
       fprintf(stdout,"%.20lf %.20lf\n",det1,rmse);
      for(i=0;i<n;i++){
	fprintf(stdout,"%.20lf ",parameters[i]);
      }
      fprintf(stdout,"\n");
